...
const Inventory = () => {
  ...
  const productSummary = useMemo(() => {
    ...
    batches.forEach((batch) => {
      ...
      const nextExpiry = batch.expiry_date;

      if (!existing) {
        summaryMap.set(key, {
          productId: key,
          name: batch.product.name,
          totalQuantity: Number(batch.remaining_quantity ?? 0),
          unit: batch.unit || "uds",
          nextExpiry,
          status: batch.status,
        });
      } else {
        existing.totalQuantity += Number(batch.remaining_quantity ?? 0);
        if (!existing.nextExpiry || (nextExpiry && nextExpiry < existing.nextExpiry)) {
          existing.nextExpiry = nextExpiry;
        }
        if (batch.status === "critical" || batch.status === "expired") {
          existing.status = batch.status;
        } else if (batch.status === "warning" && existing.status === "ok") {
